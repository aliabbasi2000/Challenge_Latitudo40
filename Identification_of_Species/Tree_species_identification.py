...
cursor.execute('DELETE FROM tree_classification')
...
# Process images from the test-dataset folder
image_extensions = ['.jpeg', '.jpg', '.png']
test_folder = 'test-dataset'
image_files = [f for f in os.listdir(test_folder) if os.path.isfile(os.path.join(test_folder, f)) and os.path.splitext(f)[1].lower() in image_extensions]

for idx, image_file in enumerate(image_files, start=1):
    image_path = os.path.join(test_folder, image_file)
    id, species, latitude, longitude = classify(model, image_transforms, image_path, classes, idx)
